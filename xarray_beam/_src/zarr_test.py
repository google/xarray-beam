...
class DatasetToZarrTest(test_util.TestCase):
  ...
  @parameterized.named_parameters(
      {'testcase_name': 'stage_locally_true', 'stage_locally': True},
      {'testcase_name': 'stage_locally_false', 'stage_locally': False},
      {'testcase_name': 'stage_locally_none', 'stage_locally': None},
  )
  def test_setup_zarr(self, stage_locally):
    dataset = xarray.Dataset(
        {'foo': ('x', np.arange(0, 60, 10))},
        coords={'x': np.arange(6)},
    )
    ...
    temp_dir = self.create_tempdir().full_path
    xbeam.setup_zarr(
        template, temp_dir, zarr_chunks={'x': 3}, stage_locally=stage_locally
    )
